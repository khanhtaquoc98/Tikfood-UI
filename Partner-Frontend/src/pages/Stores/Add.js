...
import FindAddress from "../../components/Store/FindAddress"
...
export default class AddStore extends React.Component{
    ...
    handleSubmit = (event) => {
        ...
        if(cookies.get('partner') !== undefined){
            if(name != ''){
                addStore(cookies.get('partner'), name, min_price, max_price, address_detail, district_id, district, category_id,
                longtitude, latitude, time_open, short_decription, list_images, introduction, sale_detail, menu).then(res => {
                   ...
                   openNotificationWithIcon('success', 'Gửi thông tin cửa hàng thành công','Vui lòng đợi quản trị viên duyệt cửa hàng của bạn nhé','bottomRight')
                }).catch(err =>  {
                    this.setState({loading: false})
                        ...
                        <div className="row">
                            <div className="col-md-8">
                                <GoogleMap
                                    ...
                                    center={{lat: this.state.latitude ? this.state.latitude : 10.7779352, lng: this.state.longtitude ? this.state.longtitude : 106.7020098}}
                                    zoom={16}
                                >
                                    
                                    <>
                                    {
                                        (this.state.latitude && this.state.longtitude) &&  <Marker
                                        position={{lat: this.state.latitude , lng: this.state.longtitude}}
                                        />
                                    }
