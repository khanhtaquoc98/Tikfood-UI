...
export const FetchStoreofPartner =  (token) => {
    return axios.get(API_URL + "/user/partner",
    {
        headers: {
            'auth-token' : token
        }
...
export const addStore = (token, name, min_price, max_price, address_detail, district_id, district, category_id, longtitude, latitude, time_open, short_decription, list_images, introduction, sale_detail, menu) => {
    return axios.post(API_URL+ "/partner/storepartner/newstore",{
            'name': name,
...
export const putStore =  (token,idStore, name, min_price, max_price, address_detail, district_id, district, category_id, longtitude, latitude, time_open, short_decription, list_images, introduction, sale_detail, menu) => {
    return axios.put(API_URL + "/user/partner/store_id=" + idStore,
    {
        'name': name,
        'min_price': Number(min_price),
        'max_price': Number(max_price),
...
export const FetchRestaurantswithLocation = (long, lat) => {
    return axios.get(API_URL + "/store/vitri?longtitude=" + long + "&latitude=" + lat).then(res => res).catch(err => null)
}

export const CheckStoreInDatabase = (idStore, idUser) => {
    return axios.post(API_URL_2 + "/v2/partner?store_id=" + idStore + "&user_id=" + idUser).then(res => res).catch(err => null)
}