...
export default class Header extends React.Component{
    ...
    isLogin = () => {
        if(cookies.get('partner') != undefined){
            ...
            GetUserWToken(cookies.get('partner')).then(res => this.setState({User : res.user}))
        } 
            ...
            <div className="d-flex">
              ...
              {this.state.User && 
                <button type="button" className="btn header-item waves-effect" id="page-header-user-dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <img className="rounded-circle header-profile-user" src={this.state.User.avatar == "Chưa cập nhật" ? "/assets/avatar.jpg" : this.state.User.avatar} alt="Avatar" />
                  <span className="d-none d-xl-inline-block ml-1">{this.state.User.fullname != "Chưa cập nhật" ? this.state.User.fullname : this.state.User.email}</span>
