...
import {putDisTrict, getDetailCategory} from '../../API/Api'
  ...
  setModal2Visible = (visible,update) => {
    if(update){
      if(cookies.get('admin') != undefined){
        ...
        putDisTrict(cookies.get('admin'),this.props.CategoryEdit, this.state.nameCategory).then(res => {
            this.setState({nameCategory: undefined, loading: false})
            this.props.setModalVisiable(visible, update)
            openNotificationWithIcon('success', 'Bạn vừa chỉnh sửa khu vực thành công', '', 'bottomRight');
