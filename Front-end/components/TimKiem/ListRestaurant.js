                ...
                <div className="listing-item-container init-grid-items fl-wrap">
                     <div className="container">
                             ...
                             ListRestaurants.map((item, index)=> {
                                return(
                                   ...
                                   <article className="geodir-category-listing fl-wrap" style={{height: "100%"}}>
                                       ...
                                       <Link href={`/${item._source.name_linkurl_store}/${item._id}`}><a  className="geodir-category-img-wrap fl-wrap">
                                           <img src={ChangeLinkImage(item._source.list_images[0])} style={{maxHeight: "227px"}}/> 
                                       </a></Link>
                                       <div className="geodir-category-opt">
                                           <div className="listing-rating-count-wrap">
                                           <Rate disabled style={{fontSize: ".8rem"}} className="listing-rating card-popup-rainingvis" allowHalf defaultValue={item._source.average_score_comment} />
                                           <br />
                                               <div className="reviews-count">{item._source.list_comment_id.length} đánh giá</div>
                                           </div>
                                       </div>
                                       </div>
                                       ...
                                       <div className="geodir-category-content-title fl-wrap">
                                           <div className="geodir-category-content-title-item" style={{height: this.props.isGridView ?  "106px" : "83px"}}>
                                               <h3 className="title-sin_map">
                                                   <Link href={`/${item._source.name_linkurl_store}/${item._id}`}>
                                                    {  this.showTitle(item._source.name) }
                                                    </Link>
                                                    {(item._source.partner != undefined  && item._source.partner.length > 0) &&<span class="verified-badge"><i class="fal fa-check"></i></span>}</h3>
                                               <div className="geodir-category-location fl-wrap"><a className="map-item"><i className="fas fa-map-marker-alt" />{item._source.address.address_detail}</a></div>
                                               <div className="geodir-category-location fl-wrap"><a className="map-item"><i className="fal fa-clock" />{item._source.time_open}</a></div>
                                               {this.showPrice(item._source.min_price, item._source.max_price)}
                                           </div>
                                       </div>
                                       <div className="geodir-category-text fl-wrap">
                                        <p className="small-text">{item._source.short_decription}</p>
                                   ...
                                   <span>{item._source.category.name}</span>
                                   </Tooltip>
                                   ...
                                   <span>{item._source.positive_sentimet_quantity}</span>
                                   </Tooltip>
                                   ...
                                   <span>{item._source.nagative_sentimet_quantity}</span>
                                   </Tooltip>   
                                </a>
                                           <div className="geodir-opt-list">
                                           <ul className="no-list-style">
                                               <li><a className="map-item"><i className="fal fa-map-marker-alt" onMouseOver={() => this.showMarker(item._id, item._source.address.latitude, item._source.address.longtitude)}/> </a></li>
                                               <li>
                                               </li>
                                           </ul>
                                           </div>
                         ...
                         {/* listing-item end */}
                         {
                             ...
                             prevPageText= {<div style={{padding: "0 30px"}}><i className="fas fa-caret-left" /><span style={{marginLeft: "5px"}}>Prev</span></div>}
                             nextPageText = { <div style={{padding: "0 30px"}}><span style={{marginRight: "5px"}}>Next</span><i className="fas fa-caret-right" /></div>}
